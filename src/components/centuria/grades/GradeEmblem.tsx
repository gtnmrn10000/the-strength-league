...
export function GradeEmblem({ grade, size = 48, state, locked = false, active = false, progress, animated = false, context = "static", className = "" }: {
  ...
  return <motion.span ref={rootRef} className={`relative inline-flex shrink-0 items-center justify-center ${className}`} style={{ width: size, height: size }} role="img" aria-label={`Grade ${GRADE_LABELS[grade]}${resolvedLocked ? ", verrouillé" : resolvedActive ? ", actuel" : ", débloqué"}`} whileTap={!reduceMotion && grade === "gladiateur" && !resolvedLocked ? { scale: .985, rotate: -.35 } : undefined}>
    <svg viewBox="0 0 72 72" width={size} height={size} aria-hidden="true" focusable="false">
      ...
      {grade === "legende" && canAnimate && <motion.circle cx="36" cy="36" r="31" fill="none" stroke={metal.detail} strokeWidth=".7" initial={{ opacity: 0, scale: .96 }} animate={{ opacity: [.04, .18, .08], scale: 1 }} transition={{ delay: context === "level-up" ? .8 : .08, duration: .8 }} style={{ transformOrigin: "36px 36px" }} />}
      {Array.from({ length: rings }).map((_, i) => <motion.circle key={i} cx="36" cy="36" r={34 - i * 2.6} fill={i === 0 ? "#090a0b" : "none"} stroke={resolvedLocked ? "#303335" : i === 0 ? metal.edge : metal.detail} strokeWidth={i === 0 ? 1.15 : .65} opacity={i === 0 ? 1 : .68} pathLength="1" initial={canAnimate ? { pathLength: 0, opacity: 0 } : undefined} animate={canAnimate ? { pathLength: 1, opacity: i === 0 ? 1 : .68, scale: grade === "spartiate" ? [1, 1.012, 1] : 1 } : undefined} transition={{ delay: context === "level-up" ? .76 + i * .12 : i * .08, duration: .62, ease: "easeOut" }} style={{ transformOrigin: "36px 36px" }} />)}
      <g opacity={resolvedLocked ? .2 : 1}><Insignia grade={grade} fill={fill} metal={resolvedLocked ? { ...metal, edge: "#4a4e50", detail: "#55595b" } : metal} animated={canAnimate} context={context} /></g>
