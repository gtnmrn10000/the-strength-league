...
comment on table public.analytics_events is 'First-party product analytics. props must never contain PII. See docs/analytics.md.';
...
begin
  ...
  select count(*) into recent_count from public.analytics_events
    where user_id = new.user_id and created_at > now() - interval '1 hour';
  if recent_count >= 300 then raise exception 'Rate limit exceeded: max 300 analytics events per hour per user'; end if;
  return new;
end; $$;
drop trigger if exists analytics_events_rate_limit on public.analytics_events;
create trigger analytics_events_rate_limit before insert on public.analytics_events for each row execute function public.enforce_analytics_rate_limit();
...
comment on table public.client_errors is 'Client-side error reports. No tokens, secrets, emails or query strings. See docs/analytics.md.';
...
create policy "client_errors_select_own" on public.client_errors for select to authenticated using (user_id = auth.uid());
...
begin
  ...
  select count(*) into recent_count from public.client_errors
    where user_id = new.user_id and created_at > now() - interval '1 hour';
  if recent_count >= 30 then raise exception 'Rate limit exceeded: max 30 client error reports per hour per user'; end if;
  return new;
end; $$;
drop trigger if exists client_errors_rate_limit on public.client_errors;
create trigger client_errors_rate_limit before insert on public.client_errors for each row execute function public.enforce_client_errors_rate_limit();